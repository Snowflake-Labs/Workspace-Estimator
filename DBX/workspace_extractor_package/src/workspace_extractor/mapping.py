...
class Mapping:
    ...
    def get_clusters_ids_from_runs(output=None):
        ...
        if len(runs_list) > 0:
            ...
            result_df = result_df[
                (result_df["duration"] > 0) & (result_df["result_state"] == "SUCCESS")
            ]
            result_df["rank"] = result_df.groupby(["run_name"])["end_time"].rank(
    ...
    def get_clusters_ids_from_clusters(output=None):
        ...
        if len(cluster_list) > 0:
            ...
            ui_clusters = result_df[result_df["cluster_source"] != "JOB"][
                "cluster_id"
            ].values
        ...
        for f in files:
            with open(f, "r") as file:
                ...
                for cluster in clusters:
                    ...
                    cluster_source = (
                        cluster.get("cluster_source", None) if cluster else None
                    )
                    tags = cluster.get("default_tags", None) if cluster else None
                    job_id = (
                        tags.get("JobId", "NO_ID_FOUND") if tags else "NO_TAG_FOUND"
                    )
                    run_name = (
                        tags.get("RunName", "NO_NAME_FOUND") if tags else "NO_TAG_FOUND"
                    )
                    start_time = cluster.get("start_time", 0)
                    end_time = cluster.get("end_time", 0)
                    termination_reason = cluster.get("termination_reason")
                    result_state = (
                        termination_reason.get("type") if termination_reason else None
                    )
                    result.append(
                        {
                ...
                for run in runs:
                    ...
                    if "tasks" in run:
                        ...
                        for task in tasks:
                            ...
                            cluster_id = (
                                cluster_instance.get("cluster_id", None)
                                if cluster_instance
                                else existing_cluster_id
                            )
                            state = task.get("state", None)
                            result_state = (
                                state.get("result_state", None) if state else None
                            )
                            if run_name:
                                run_name = (
                                    run_name[:-37] if "ADF" in run_name else run_name
                                )
                            result.append(
                                {
                                    "run_name": Util.get_clean_name(run_name),
